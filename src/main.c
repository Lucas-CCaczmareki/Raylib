...
//Basicamente cria formas padronizadas de identificar o controle conectado
...
int main() {
    ...
    //Seta uma suavização de bordas. Anti-aliasing
    SetConfigFlags(FLAG_MSAA_4X_HINT);

    InitWindow(screenWidht, screenHeight, "Mapeando Controle de vídeogame");

    //Carrega as imagens dos controles como texturas
    Texture2D textPs3Pad = LoadTexture("resources/ps3.png");
    Texture2D textXboxPad = LoadTexture("resources/xbox.png");
    ...
    /*
    A variável gamepad permite alternar entre diferentes controles conectados no computador
    Ela é um índice que identifica, e inicia no primeiro controle identificado

    Para alternar entre controles, use as setinhas. <- e ->
    */
    int gamepad = 0; 
    ...
    while(!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            //Parte do código que alterna entre controles
            //Aqui dava pra melhorar vendo se tem algum controle conectado antes de alternar.
                //Mas por enquanto ta OK
            if (IsKeyPressed(KEY_LEFT) && gamepad > 0) gamepad--;
            if (IsKeyPressed(KEY_RIGHT)) gamepad++;

            // Executa o código que mapeia o controle se for detectado algum controle.
            // A conexão entre gamepad e os controles é gerenciada internamente pelo Raylib.
            // Um índice é atribuído na ordem que os controles são conectados no sistema.
            if (IsGamepadAvailable(gamepad)) {
                ...
                //Calculate deadzones
                //Se o controle estiver na nossa deadzone, o valor que ele lê retorna 0
                ...
                //nos casos dos triggers, -1.0 é o valor de repouso deles
                if (leftTrigger < leftTriggerDeadzone) leftTrigger = -1.0f;
                if (rightTrigger < rightTriggerDeadzone) rightTrigger = -1.0f;

                //Verifica se o controole conectado é de xbox.
                if (TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_1) > -1 || TextFindIndex(TextToLower(GetGamepadName(gamepad)), XBOX_ALIAS_2) > -1) {
                    //Desenha a imagem que representa o controle do xbox.
                    DrawTexture(textXboxPad, 0, 0, DARKGRAY);

                    //Essa parte preenche os espaços em branco na imagem do controle se algum dos botões for selecionado.
                    ...
                    /*
                     ESCREVER ESSE COMENTÁRIO NA PRÓXIMA VEZ!!!
                    */
                    ...
                    //Cria uma imagem responsiva ao quão pressionado os gatilhos estão
                    //Não entendi muito bem as conversões ainda tho. Vale a pena revisar.
                ...
                // -------------------------------------------------------------------------------------------------------
                    ...
                    DrawTexture(textPs3Pad, 0, 0, DARKGRAY);
                    ...
                    //basicamente desenha um triangulo, por isso q ta diferente.
                ...
                DrawTexture(textXboxPad, 0, 0, LIGHTGRAY);
            }
        EndDrawing();
    }

    //Finalizando o programa
    UnloadTexture(textPs3Pad);
    UnloadTexture(textXboxPad);
